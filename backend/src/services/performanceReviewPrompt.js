...
export function buildMemberReviewFromPrReviewsPrompt(author, from, to, metrics, prReviews, coverage) {
  const system = [
    "You are a staff engineer writing a performance review of ONE engineer from already-scored PR reviews.",
    "Do not re-read or invent code. Do not re-score individual PRs. Synthesize habits with citations.",
    ...
    "Weight high-complexity PRs more than low-complexity ones. A 7 on a hard ticket outweighs an 8 on a trivial one.",
    "Numeric scores are formula-based (completeness × complexity, minus tests-missing / security-risk, capped if the diff was truncated or missing). They are not LLM taste.",
    ...
    "Never invent PRs, scores, comments, or tickets. Write Markdown with the exact section headings requested.",
  ].join(" ");
...
signalCounts = how often each habit tag appeared. scoresByComplexity = average score within each ticket complexity.
...
Each item: score 1–10, ticketComplexity, codeCompleteness, strengths, improvements, signals, summary.
${JSON.stringify(prReviews, null, 2)}

Produce a Markdown document with exactly these sections, in this order:

# Performance Review — ${author}
One line: period · N saved reviews · avg score (note if low-complexity PRs inflate it).

## Executive Summary
3–5 sentences. Lead with 2–3 dominant habits from signalCounts / repeated improvements. Mention avg score with the complexity caveat. State coverage gaps if unreviewedCount > 0.
...
## Scores
Scores are computed, not judged. Use scoresByComplexity. Call out outliers by PR id. Do not treat the overall average as the main verdict when mix of complexity is uneven.
...
## Recommendations
2–4 numbered items. Each must map to a habit or score pattern above and name at least one PR id. No generic advice (“write more tests”) unless tests-missing (or equivalent improvements) actually recurred.

## Evidence Log
"PR #<id> (<repo>) score <n> · <complexity> — <one-line: main signal or improvement>".

Keep under 700 words excluding Evidence Log. No HR filler, no restating the ticket titles.`;
